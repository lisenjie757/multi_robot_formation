...
int main(int argc, char** argv){
	...
	ros::Subscriber laser = followerNode.subscribe("/tb3_2/scan",10,laserCallBack);
	...
	while(followerNode.ok()){
		...
		if(name.name == 0){
			...
			vel_msg.angular.z = 4.0 * atan2(transform_sm.getOrigin().y(),transform_sm.getOrigin().x());
                	vel_msg.linear.x = 0.2 * sqrt(pow(transform_sm.getOrigin().x(),2) + pow(transform_sm.getOrigin().y(),2));
		}else if(name.name == 1){
			//triangle
			...
			//line
			vel_msg.angular.z = 4.0 * atan2(transform_sm.getOrigin().y(),transform_sm.getOrigin().x());
        	        vel_msg.linear.x = 0.15 * sqrt(pow(transform_sm.getOrigin().x(),2) + pow(transform_sm.getOrigin().y(),2));
