...
int main(int argc, char** argv){
	...
	slave_vel = followerNode.advertise<geometry_msgs::Twist>("/tb3_3/cmd_vel",10);
	ros::Subscriber laser = followerNode.subscribe("/tb3_3/scan",10,laserCallBack);
	...
	while(followerNode.ok()){
		...
		try{
			//listener.waitForTransform("/tb3_3/base_footprint","/tb3_0/base_footprint",ros::Time(0),ros::Duration(10.0));
			listener.lookupTransform("/tb3_3/base_footprint","/tb3_0/base_footprint",ros::Time(0),transform_sm);
		...
		try{
			//listener.waitForTransform("/tb3_0/base_footprint","/tb3_3/base_footprint",ros::Time(0),ros::Duration(10.0));

                        listener.lookupTransform("/tb3_0/base_footprint","/tb3_3/base_footprint",ros::Time(0),transform_ms);
                }catch(tf::TransformException& ex){
                        ROS_ERROR("%s",ex.what());
                        ros::Duration(1.0).sleep();
                        continue;
		...
		if(name.name == 0){
			...
			vel_msg.angular.z = 4.0 * atan2(transform_sm.getOrigin().y(),transform_sm.getOrigin().x());
                	vel_msg.linear.x = 0.1 * sqrt(pow(transform_sm.getOrigin().x(),2) + pow(transform_sm.getOrigin().y(),2));
		}else if(name.name == 1){
			//triangle
